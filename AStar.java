...
public class AStar {
        ...
        public static HashMap<Integer, Double> hVals = new HashMap<>();
        ...
        public void createHeuristic(int end){
            hVals.clear();
            for(int i =0; i <nodes.size(); i++){
                Node n = nodes.get(i);
                double hVal = n.distTo(nodes.get(end));
                hVals.put(i,hVal);
            }
        }
        ...
        public void start(int startNode, int end) {
            ...
            while(currNode != end) {
                ...
                for(int node : currentConnect) {
                    double dist = (currAccNode.distTo(nodes.get(node)) + distances[currNode]) + hVals.get(node);
                    if(dist < distances[node]) {
                        distances[node] = dist - hVals.get(node);
                        prev[node] = currNode;
                    }
                }
